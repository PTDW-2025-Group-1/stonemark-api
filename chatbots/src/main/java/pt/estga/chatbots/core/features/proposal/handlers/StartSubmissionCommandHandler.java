...
import pt.estga.chatbots.core.features.proposal.commands.StartSubmissionCommand;
...
public class StartSubmissionCommandHandler implements CommandHandler<StartSubmissionCommand> {

    private final Cache<String, ConversationContext> conversationContexts;

    @Override
    public BotResponse handle(StartSubmissionCommand command) {
        ConversationContext context = conversationContexts.get(command.getInput().getUserId(), k -> new ConversationContext());
        context.setCurrentStateName("WAITING_FOR_PHOTO");
