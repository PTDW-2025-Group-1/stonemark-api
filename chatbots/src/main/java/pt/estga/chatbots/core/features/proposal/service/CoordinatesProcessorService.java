    ...
    public BotResponse processCoordinates(ConversationContext context) {
        ...
        if (proposal.getLatitude() != null && proposal.getLongitude() != null) {
            Menu coordinatesMenu = Menu.builder()
                    ...
                    .buttons(List.of(
                            List.of(
                                    Button.builder().text("📍 Use detected coordinates").callbackData(CallbackData.USE_DETECTED_COORDINATES).build(),
                                    Button.builder().text("🖐 Send location manually").callbackData(CallbackData.SEND_LOCATION_MANUALLY).build()
                            )
                    ))
                    .build();
